...
func (u *Uow) Do(ctx context.Context, fn func(Uow *Uow) error) error {
	...
	if err = fn(u); err != nil {
		if errRb := u.Rollback(); errRb != nil {
			return errors.New(fmt.Sprintf("error Rollback: %s, Error Transaction: %s", errRb.Error(), err.Error()))
		}
		return err
	}
...
func (u *Uow) CommitOrRollback() error {
	...
	if err != nil {
		errRb := u.Rollback()
		if errRb != nil {
			return fmt.Errorf("error Rollback: %s, error Transaction: %s", errRb.Error(), err.Error())
		}
		return err
	}
	u.Tx = nil
